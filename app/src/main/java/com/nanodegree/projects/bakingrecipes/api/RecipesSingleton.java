 ...
 * Use third party library Retrofit to get Json from API response
 */
public final class RecipesSingleton implements Serializable {

    private static volatile RecipesSingleton recipesSingleton = new RecipesSingleton();
    private BakingJsonResponse bakingJsonResponse;
    ...
    private RecipesSingleton() {
        ...
        bakingJsonResponse = retrofit.create(BakingJsonResponse.class);
    }

    public static RecipesSingleton getInstance() {
        if (recipesSingleton == null) {
            synchronized (RecipesSingleton.class) {
                if (recipesSingleton == null) recipesSingleton = new RecipesSingleton();
            }
        }
        return recipesSingleton;
    }

    public void getRecipesFromApiCall(final RecipesApiCallback<List<Recipe>> recipesApiCallback) {
        bakingJsonResponse.getRecipesResponse().enqueue(new Callback<List<Recipe>>() {
